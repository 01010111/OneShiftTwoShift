...
Void FlxKeyboard_obj::onKeyUp( ::openfl::_legacy::events::KeyboardEvent event){
{
		...
		if ((tmp4)){
			...
			if ((tmp11)){
				...
				HX_STACK_LINE(102)
				tmp13->set_stageFocusRect(false);
				HX_STACK_LINE(102)
				...
				HX_STACK_LINE(102)
				tmp15->set_focus(tmp16);
			}
			HX_STACK_LINE(102)
			::flixel::_system::frontEnds::DebuggerFrontEnd tmp12 = ::flixel::FlxG_obj::debugger;		HX_STACK_VAR(tmp12,"tmp12");
			HX_STACK_LINE(102)
			tmp12->__FieldRef(HX_HCSTRING("visible","\x72","\x78","\x24","\xa3")) = Value;
		}
	}
return null();
}
